...
class GameScene: SKScene {
    ...
    var gameType = GameType.soloMatch
    ...
    override func didMove(to view: SKView) {
        ...
        case .remoteMatch:
            ...
            NotificationCenter.default.addObserver(
                self,
                selector: #selector(processGameCenterRequest),
    ...
    func setupRemoteGame() {
        ...
        if !GameKitHelper.shared.canTakeTurn() {
    ...
    func processEndTurnForRemoteGame() {
        ...
        GameKitHelper.shared.endTurn(gameCenterData!) { error in
            if let error = error {
                print("processEndTurnForRemoteGame, error from endTurn \(error)")
            }
        }
    ...
    func endGame() {
        ...
        if gameType == .remoteMatch {
            if isWinner {
                GameKitHelper.shared.winGame(gameCenterData!) { error in
                    if let error = error { print("error returned from winGame: \(error)") }
                }
            } else {
                GameKitHelper.shared.lostGame(gameCenterData!) { error in
                    if let error = error { print("error returned from lostGame: \(error)") }
                }
            }
        }
    
        loadGameOverScene(isWinner: isWinner)
    }
